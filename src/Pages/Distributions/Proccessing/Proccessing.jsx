...
import ProccessingCard from "../../../Card/ActivityCard/ProccessingCard";
...
const Processing = () => {
  const {
    ...
    queryFn: async () => {
      const response = await axios.get(`${BASEURL}/admin/processing`, {
        headers: {
      ...
      <div className="ll">
        ...
        <div className="client_container flex flex-col gap-2 mt-2">
          {inspectionData?.data?.map((music, i) => {
            return <ProccessingCard key={i} music={music}></ProccessingCard>;
          })}
        </div>
      </div>
    </div>

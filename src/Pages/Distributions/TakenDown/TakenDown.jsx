...
import TakenDownCard from "../../../Card/ActivityCard/TakenDownCard";
...
const TakenDown = () => {
  const {
    ...
    queryFn: async () => {
      const response = await axios.get(`${BASEURL}/admin/take-down`, {
        headers: {
      ...
      <div className="ll">
        ...
        <div className="client_container flex flex-col gap-2 mt-2">
          {inspectionData?.data?.map((music, i) => {
            return <TakenDownCard key={i} music={music}></TakenDownCard>;
          })}
        </div>
      </div>
    </div>
